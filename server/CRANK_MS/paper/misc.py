...
def get_metrics(y_true_all, y_preds_all, scores_all):
    ...
    for y_true, y_pred, scores in zip(y_true_all, y_preds_all, scores_all):
        ...
        P = (TP + FP) / N
        acc = (TN + TP) / N
        sen = TP / (TP + FN + epsilon)  # Add epsilon to avoid division by zero
        spc = TN / (TN + FP + epsilon)  # Add epsilon to avoid division by zero
        prc = TP / (TP + FP + epsilon)  # Add epsilon to avoid division by zero
        f1s = 2 * (prc * sen) / (prc + sen + epsilon)  # Add epsilon to avoid division by zero
        mcc = (TP / N - S * P) / np.sqrt(P * S * (1 - S) * (1 - P) + epsilon)  # Add epsilon
